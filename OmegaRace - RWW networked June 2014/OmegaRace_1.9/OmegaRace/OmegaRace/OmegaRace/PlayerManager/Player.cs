...
namespace CollisionManager
{
    ...
    class Player
    {
        ...
        public void createMissile(Game1 g)
        {
            ...
            if (g.gamer.IsHost) 
            {
                CreateMissileMsg msg = new CreateMissileMsg(id, missile.id);
        ...
        public void doWork(Player_Data data, Game1 g)
        {
            switch (data.op)
            {
                case PlayerOp.rotate:
                    shipData.op = ShipOp.rotate;
                    shipData.input = data.input;
                    playerShip.doWork(shipData);
                    //playerShip.spriteRef.body.Rotation += data.input;
                    break;
                case PlayerOp.accelerate:
                    /*
                    Vector2 direction = new Vector2((float)(Math.Cos(playerShip.spriteRef.body.GetAngle())), (float)(Math.Sin(playerShip.spriteRef.body.GetAngle())));
                    direction.Normalize();
                    direction *= data.input;
                    playerShip.spriteRef.body.ApplyLinearImpulse(direction, playerShip.spriteRef.body.GetWorldCenter());
                     * */
                    shipData.op = ShipOp.accelerate;
                    shipData.input = data.input;
                    playerShip.doWork(shipData);
                    break;
                case PlayerOp.fireMissile:
                    createMissile(g);
                    break;
            }
        }
